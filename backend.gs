...
function tryOrLog(fn, label) {
  try { fn(); } catch (e) { Logger.log('ERRO ' + label + ': ' + e.toString()); }
}
...
function parseIngressosStr(ingressosStr, total) {
  const items = [];
  const totalNum = parseFloat(String(total).replace('R$', '').replace(/\s/g, '').replace(',', '.')) || 0;
  String(ingressosStr || '').split(', ').forEach(p => {
    const m = p.match(/(.+?) ×(\d+)(?: \(R\$ ([\d.,]+)\))?/);
    if (!m) return;
    const qty = parseInt(m[2]);
    let preco = 0;
    if (m[3]) {
      const subtotal = parseFloat(m[3].replace('.', '').replace(',', '.'));
      preco = qty > 0 ? subtotal / qty : 0;
    }
    items.push({ tipo: m[1], quantidade: qty, preco: preco });
  });
...
function paymentIdExisteNaPlanilha(paymentId) {
  try {
    ...
    const col = sheet.getRange(2, 19, sheet.getLastRow() - 1, 1).getValues();
    const found = col.some(row => String(row[0]) === String(paymentId));
    cache.put(cacheKey, found ? '1' : '0', 300);
    return found;
  } catch (e) {
    Logger.log('paymentIdExisteNaPlanilha erro: ' + e.toString());
    return false;
  }
}
...
function fetchPaymentStatus(paymentId) {
  try {
    ...
    Logger.log('fetchPaymentStatus ' + paymentId + ': ' + e.toString());
    return null;
  }
}

/**
 * Endpoint principal — recebe POST do frontend (CC reduzida).
 */
function doPost(e) {
  try {
    const data = JSON.parse(e.postData.contents);
    Logger.log('doPost recebido — action=' + (data.action || 'payment') +
    ...
    const validation = validateDoPostPayload(data);
    if (!validation.ok) return jsonOut({ status: 'rejected', message: validation.message });

    const { inscrito, items, paymentData, total } = validation;

    // Lock curto: só para verificar vagas. Libera antes do UrlFetch ao MP (que pode levar 3-5s).
    const lock = LockService.getScriptLock();
    try { lock.waitLock(15000); }
    catch (lockErr) { return jsonOut({ status: 'rejected', message: 'Servidor ocupado. Tente novamente em alguns segundos.' }); }

    try {
      const limite = verificarLimiteVagas(items);
      if (!limite.ok) return jsonOut({ status: 'rejected', message: limite.message });
    } finally {
      try { lock.releaseLock(); } catch (e) {}
    }

    const paymentResult = processPayment(paymentData, inscrito, items, total);
    return routePaymentResult(paymentResult, inscrito, items, total);
...
function validateDoPostPayload(data) {
  const { paymentData, inscrito, items } = data;
  if (!paymentData || !inscrito || !items || !items.length)
    return { ok: false, message: 'Dados incompletos. Preencha todos os campos e tente novamente.' };
  if (inscrito.cpf && !checkRateLimit(inscrito.cpf))
  ...
  const p = validarPrecos(items);
  if (!p.ok) return { ok: false, message: p.message };
  return { ok: true, paymentData, inscrito, items, total: p.total };
}

function routePaymentResult(result, inscrito, items, total) {
  if (result.status === 'approved') return handleApproved(result, inscrito, items, total);
  if (result.status === 'pending' && result.payment_method_id === 'pix')
    return handlePixPending(result, inscrito, items, total);
  if (result.status === 'in_process' || result.status === 'pending')
    return handleInProcess(result, inscrito, items, total);
  return handleRejected(result, inscrito, items, total);
}

function handleApproved(result, inscrito, items, total) {
  tryOrLog(() => saveToSheet(inscrito, items, total, result), 'saveToSheet approved');
  tryOrLog(() => sendConfirmationEmail(inscrito, items, total, result.id), 'email approved');
  return jsonOut({ status: 'approved', payment_id: result.id, message: 'Pagamento aprovado com sucesso!' });
}

function handlePixPending(result, inscrito, items, total) {
  tryOrLog(() => saveToSheet(inscrito, items, total, result, 'Aguardando Pix'), 'saveToSheet pix');
...
function handleInProcess(result, inscrito, items, total) {
  tryOrLog(() => saveToSheet(inscrito, items, total, result, 'Em Análise'), 'saveToSheet review');
  tryOrLog(() => sendReviewEmail(inscrito, items, total, result.id), 'email review');
  return jsonOut({
    status: 'in_process',
    payment_id: result.id,
...
function handleRejected(result, inscrito, items, total) {
  let statusPlanilha = 'Recusado';
  if (result.status === 'cancelled') statusPlanilha = 'Cancelado';
  else if (result.status === 'rejected') statusPlanilha = 'Recusado';
  else if (result.status) statusPlanilha = 'Recusado — ' + result.status;

  tryOrLog(() => saveToSheet(inscrito, items, total, result, statusPlanilha), 'saveToSheet rejected');
  tryOrLog(() => sendRejectedEmail(inscrito, items, total, result), 'email rejected');
 ...
 * Proteções:
 *  - Valida paymentId
 *  - Confirma que o ID existe na planilha (evita enumeração pública)
 *  - Cache de 10s para reduzir pressão do polling 3s do frontend
 *  - Não vaza err.toString() ao cliente
 */
function checkPixPayment(paymentId) {
  ...
  try {
    ...
    if (!result) return jsonOut({ status: 'error', message: 'Falha ao consultar Mercado Pago' });

    Logger.log('Pix check — id=' + paymentId + ' status=' + result.status);

    if (result.status === 'approved') {
      tryOrLog(() => aprovarPixNaPlanilha(paymentId), 'aprovarPixNaPlanilha');
    } else if (['cancelled', 'rejected', 'expired'].indexOf(result.status) >= 0) {
      tryOrLog(() => marcarPixExpiradoNaPlanilha(paymentId, result.status), 'marcarPixExpirado');
    }
    ...
    Logger.log('Erro checkPixPayment id=' + paymentId + ': ' + err.toString());
    return jsonOut({ status: 'error', message: 'Erro ao verificar pagamento' });
  }
}

function aprovarPixNaPlanilha(paymentId) {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) return;
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    if (String(data[i][18]) !== String(paymentId) || data[i][17] !== 'Aguardando Pix') continue;
    ...
    const items = parseIngressosStr(data[i][10], data[i][12]);

    tryOrLog(
      () => sendConfirmationEmail({ nome: nome, email: email }, items, data[i][12], paymentId),
      'email Pix aprovado'
    );
    return;
  }
...
function marcarPixExpiradoNaPlanilha(paymentId, statusMp) {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) return;
...
function reverificarPixPendentes() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) { Logger.log('Planilha não encontrada'); return; }

  const data = sheet.getDataRange().getValues();
  const updates = [];        // { row, value }
  const emailsAprovado = []; // { nome, email, items, total, paymentId }
  const emailsExpirado = []; // { nome, email, paymentId }
  let verificados = 0;

  for (let i = 1; i < data.length; i++) {
    const status = data[i][17];
    const paymentId = data[i][18];
    if (!((status === 'Aguardando Pix' || status === 'Em Análise') && paymentId)) continue;

    verificados++;
    const result = fetchPaymentStatus(paymentId);
    if (!result) continue;

    Logger.log('  → id=' + paymentId + ' status=' + result.status);

    if (result.status === 'approved') {
      updates.push({ row: i + 1, value: 'Aprovado' });
      emailsAprovado.push({
        nome: data[i][1], email: data[i][2],
        items: parseIngressosStr(data[i][10], data[i][12]),
        total: data[i][12], paymentId: paymentId,
      });
      continue;
    }

    if (['cancelled', 'rejected', 'expired'].indexOf(result.status) >= 0) {
      if (status === 'Em Análise') {
        updates.push({ row: i + 1, value: 'Recusado' });
        continue;
      }
      updates.push({ row: i + 1, value: result.status === 'expired' ? 'Pix Expirado' : 'Pix Cancelado' });
      emailsExpirado.push({ nome: data[i][1], email: data[i][2], paymentId: paymentId });
      continue;
    }

    // pending ou in_process — só expira Pix acima de 35min
    if ((result.status === 'pending' || result.status === 'in_process') && status === 'Aguardando Pix') {
      const minutos = (Date.now() - new Date(data[i][0]).getTime()) / 60000;
      if (minutos > 35) {
        updates.push({ row: i + 1, value: 'Pix Expirado' });
        emailsExpirado.push({ nome: data[i][1], email: data[i][2], paymentId: paymentId });
      }
    }
  }

  // Batch write — agrupa escritas em um só flush
  updates.forEach(u => sheet.getRange(u.row, 18).setValue(u.value));
  if (updates.length) SpreadsheetApp.flush();

  emailsAprovado.forEach(e =>
    tryOrLog(() => sendConfirmationEmail({ nome: e.nome, email: e.email }, e.items, e.total, e.paymentId),
             'email aprovado (reverificar)')
  );
  emailsExpirado.forEach(e =>
    tryOrLog(() => sendPixExpiredEmail({ nome: e.nome, email: e.email }, e.paymentId),
             'email expirado (reverificar)')
  );

  Logger.log('=== RESUMO === verificados=' + verificados + ' atualizados=' + updates.length);
  tryOrLog(() => atualizarResumoVagas(), 'atualizar resumo');
}
...
function atualizarResumoVagas() {
  ...
  // Destacar linhas de total (subtotal BP + linha Experience)
  resumo.getRange('A8:F9').setBackground('#2a1f15').setFontWeight('bold');
...
function reprocessarTaxas() {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) { Logger.log('Aba de inscrições não encontrada'); return; }

  const data = sheet.getDataRange().getValues();
  const nLinhas = data.length - 1;
  if (nLinhas <= 0) return;

  // Lê o bloco existente das 3 colunas em uma chamada (linhas 2..N, colunas 15..17)
  const taxasAtuais = sheet.getRange(2, 15, nLinhas, 3).getValues();
  let atualizados = 0;
  let ignorados = 0;
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (totalBruto <= 0) { ignorados++; continue; }
    ...
    taxasAtuais[idxBlock] = [taxaPct, taxaReais, valorLiquido];
    atualizados++;
  }

  // Single batch write — 1 round-trip ao Sheets em vez de 3×N
  sheet.getRange(2, 15, nLinhas, 3).setValues(taxasAtuais);
  SpreadsheetApp.flush();

  Logger.log('=== RESUMO === atualizados=' + atualizados + ' ignorados=' + ignorados);
  tryOrLog(() => atualizarResumoVagas(), 'atualizar resumo');
}
...
function processPayment(paymentData, inscrito, items, total) {
  ...
  Logger.log('processPayment — method=' + (fd.payment_method_id || 'N/A') +
             ' installments=' + (fd.installments || 1) +
             ' total=' + total +
             ' cpf=' + maskCpf(inscrito.cpf));
  ...
  Logger.log('MP Response — id=' + result.id +
             ' status=' + result.status +
             ' detail=' + (result.status_detail || 'N/A'));
...
function saveToSheet(inscrito, items, total, paymentResult, statusOverride) {
  ...
  // Resumo é atualizado pelo trigger reverificarPixPendentes (10min) e funções
  // utilitárias — fora do hot path de doPost para reduzir latência da resposta.
}
  ...
  <div style="max-width:600px;margin:0 auto;font-family:'Segoe UI',Arial,sans-serif;background:#0e0c0a;color:#f0ebe4;border-radius:12px;overflow:hidden;border:1px solid rgba(220,120,20,.2);">
    ...
    <div style="padding:28px 24px;">
      ...
      <p style="margin-top:16px;font-size:13px;color:#5a4e42;">
        ID do pagamento: <strong style="color:#9a8a78;">#${escapeHtml(String(paymentId))}</strong>
      </p>
  ...
  <div style="max-width:600px;margin:0 auto;font-family:'Segoe UI',Arial,sans-serif;background:#0e0c0a;color:#f0ebe4;border-radius:12px;overflow:hidden;border:1px solid rgba(220,120,20,.2);">
    ...
    <div style="padding:28px 24px;">
      ...
      <p style="margin-top:16px;font-size:13px;color:#8a7a6a;">
        ID do pagamento: <strong style="color:#b0a090;">#${escapeHtml(String(paymentId))}</strong>
      </p>
  ...
  <div style="max-width:600px;margin:0 auto;font-family:'Segoe UI',Arial,sans-serif;background:#0e0c0a;color:#f0ebe4;border-radius:12px;overflow:hidden;border:1px solid rgba(220,120,20,.2);">
    ...
    <div style="padding:28px 24px;">
      ...
      <div style="background:#1a1510;border:1px solid rgba(220,120,20,.14);border-radius:10px;padding:18px 20px;margin-bottom:24px;font-size:13px;color:#b0a090;">
        ...
        <p style="margin:0;">ID do pagamento: <strong style="color:#f0ebe4;">#${escapeHtml(String(paymentId))}</strong></p>
      </div>
      ...
      <div style="background:#1a1510;border:1px solid rgba(220,38,38,.3);border-radius:10px;padding:18px 20px;margin-bottom:24px;">
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <tr>
            ...
            <td style="padding:5px 0;font-weight:600;color:#f87171;">#${escapeHtml(String(paymentId))}</td>
          </tr>
          <tr>
